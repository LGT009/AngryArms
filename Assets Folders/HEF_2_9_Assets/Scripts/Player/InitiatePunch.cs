...
public class InitiatePunch : MonoBehaviour
{
    ...
    [Header("Original Logic Variables")]
    ...
    [SerializeField] float forceMultiplier = 40f; // Increased to ensure it moves

    private float waitTime = 1.0f; 

    void Start()
    {
        // Set initial stiff state
    ...
    IEnumerator ExecutePhysicsPunch(Rigidbody rb, SpringJoint joint, Transform directionRef, float duration, bool isLeft)
    {
        ...
        // 1. LOOSEN: Allow physics to move the fist
        joint.maxDistance = 10f; // Allow the fist to travel
        ...
        // 2. EXTEND: Wait for the forward motion
        yield return new WaitForSeconds(extensionTime);

        // 3. RECOIL: Reset distance and crank up the spring to snap it back
        joint.maxDistance = 0f; 
        SetJointState(joint, idleSpring, idleDamper);
        
        // Optional: Clean up velocity after snap-back to prevent bouncing
