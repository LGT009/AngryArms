...
public class InitiatePunch : MonoBehaviour
{
    ...
    Quaternion modelRot;//same for both firsts because of the negative scale on leftParent
    ...
    void Start()
    {
        ...
        modelRot = rightFist.transform.GetChild(0).transform.rotation;
    ...
    void Update() 
    {
        ...
        if(leftCharge){//if your punch input is being held down
        ...
        if(lTimer>0f)//lower left cooldown if its above 0
        {
            lTimer-=Time.deltaTime;
            leftCanPunch = false;
        } else {//reset fist when timer runs out
            //resetModel(leftFist);
            leftCanPunch = true;
            leftHasHit = true;
        }
    ...
    /*public void resetModel(GameObject fist){
        fist.transform.GetChild(0).GetComponent<Rigidbody>().isKinematic = true;
        fist.transform.GetChild(0).transform.rotation = modelRot;
        fist.transform.GetChild(0).transform.position = Vector3.zero;
